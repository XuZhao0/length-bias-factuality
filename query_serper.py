...
# Difference: We also use titles in the snippets.
  ...
  def _parse_snippets(self, results: dict[Any, Any]) -> list[str]:
    ...
    if results.get('knowledgeGraph'):
      ...
      for attribute, value in kg.get('attributes', {}).items():
        if "Missing" in attribute and "Show results with" in value:
            continue
        else:
    ...
    if result_key in results:
      for result in results[result_key][:self.k]:
        ...
        for attribute, value in result.get('attributes', {}).items():  
          # 'Missing: birthplace | Show results with:birthplace.'
          #  Filter out missing values in Google search results, as \
          # they will interfere with the final answer generation.
          if "Missing" in attribute:
            continue
          else:
